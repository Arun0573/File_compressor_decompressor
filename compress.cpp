...
void createCharCodeArr(CharCode* charCodeArr, int* index, Node* root, int arr[], int top) {
    ...
    if (isLeaf(root)) {
       ...
       charCodeArr[*index].code = (int*) malloc(sizeof(int)*top);
...
CharCode* huffmanCodes(char data[], int freq[], int size) {
    ...
    top = 0;
    CharCode* charCodeArr = (CharCode*) malloc(sizeof(CharCode)*size);
...
CharFreq* createCharFreq(char* inputStr, int* size) {
    ...
    *size = uniqueCharCount;
    ...
    CharFreq* charFreqArr = (CharFreq*) malloc(sizeof(CharFreq)*uniqueCharCount);
    int index = 0;
    for (int i = 0; i < 256; i++) {
        if(charCount[i]) {
...
char* createKeyArray(CharFreq* charFreqArr, int size) {
    char* keyArray = (char*) malloc(sizeof(char)*size);
...
int* createFreqArray(CharFreq* charFreqArr, int size) {
    int* freqArray = (int*) malloc(sizeof(int)*size);
    for (int i = 0; i < size; i++) {
...
char* createString(int arr[], int size) {
    char* result = (char*)(char*) malloc(size + 1); // +1 for the null-terminator
    if (result == NULL) {
        return NULL;
    }
    result[0] = '\0';
...
void writeEncodedFile(CharCode* charCodeArr, const char* inputFile, const char* outputFile, int size) {
    ...
    size_t bufferSize = 1;
    bigString = (char*) malloc(bufferSize);
    bigString[0] = '\0';
    ...
    while ((ch = fgetc(input)) != EOF) {
        ...
        while (bufferSize < (strlen(bigString) + charCode.size + 1)) {
            bufferSize *= 2;
            bigString = (char*) realloc(bigString, bufferSize);
        }
    ...
    if (strlen(bigString) % 8 != 0) {
        ...
        char* temp = (char*) malloc(strlen(bigString) + padLength + 1);
        temp[0] = '\0';

        strcat(temp, padString);
        strcat(temp, bigString);
        
        bigString = (char*) realloc(temp, strlen(temp));
